...
2. The active canon in `research/canon/`.
...
For HSD, `issues/README.md` owns mutable issue status, individual issue files
own acceptance criteria and completion evidence, and `ROADMAP.md` owns phase
sequence. For an activated research study, its README owns study status and its
issues own delivery state. Root and package READMEs summarize and link; they do
not become competing status authorities.
...
At the current scale, stable file identifiers, hashes, and explicit references
are sufficient. Do not build a generic graph or registry system solely to
represent this chain.
...
5. Record significant disposition in an accepted decision record.
...
Historical Git objects and duplicated exports are provenance, not independent
corroboration.
