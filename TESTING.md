...
| HSD8-C-001 | Implemented, pending review: the official deadline, Taskmaster/stack requirements, submission artifacts, judging weights, new-project disclosure, and freeze rules were re-read and dated. | `issues/HACKATHON-CONSTRAINTS.md`; HSD-008 Completion Record |
| HSD8-R-001 | Planned: an exact clean Git archive installs and passes repository, audit, type, unit/integration/PostgreSQL/coverage/E2E, and build gates without local state. | HSD-008 clean-archive and CI records |
| HSD8-D-001, HSD8-D-002 | Planned: the judge-facing root README and canonical submission package remain machine-checked, reproducible, linked, disclosed, and consistent with the release. | `scripts/verify-release.mjs`; documentation review |
| HSD8-V-001 | Planned: a timed ≤4:00 English-ready storyboard includes friction/value, an unedited live action, Cloud proof, architecture/evidence, safety limits, and no secret capture. | `docs/submission/README.md`; manual rehearsal record |
| HSD8-Q-001 | Planned: tracked release hygiene rejects credentials, local/generated state, duplicate artifacts, broken links, unsupported placeholders, and undisclosed licence/dependency risk. | `scripts/verify-release.mjs`; `pnpm check`; `pnpm audit:prod` |
...
| HSD8-F-001 | Planned: final owner video/Devpost/tag/freeze actions are explicit and separate from completed repository work. | `docs/submission/README.md`; HSD-008 Completion Record |
