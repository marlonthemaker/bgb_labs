...
| HSD8-R-001 | Locally verified, pending CI: exact commit `658ca36` installed and passed repository, audit, type, unit/integration/coverage/E2E, and build gates in a secret-free clean Git clone; PR CI must supply the PostgreSQL 17 proof. | HSD-008 clean-clone and CI records |
| HSD8-D-001, HSD8-D-002 | Locally verified: the judge-facing root README and canonical submission package are machine-checked, reproducible, linked, disclosed, and consistent with the release. | `scripts/verify-release.mjs`; documentation review |
...
| HSD8-Q-001 | Locally verified: tracked release hygiene rejects credentials, local/generated state, duplicate artifacts, broken links, unsupported placeholders, and undisclosed licence/dependency risk. | `scripts/verify-release.mjs`; `pnpm check`; `pnpm audit:prod` |
| HSD8-O-001 | Planned: public repository and merged-main Cloud Run evidence pass URL, API, browser, configuration, traffic, and sanitized-log checks. | HSD-008 Completion Record |
