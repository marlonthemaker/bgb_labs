...
server-only test URL is present. The contract executes through a login role
with only `CONNECT`, schema `USAGE`, and table `SELECT`/`INSERT`; SQL mocks or a
database-owner connection do not satisfy HSD7-R-004.
...
| HSD7-R-001 | Locally, CI, and externally verified: memory and PostgreSQL obey one append-only/idempotent/conflict/order contract, including concurrent replay; the merged-main Cloud Run revision retrieved evidence written before its deployment. | `hotel_shoreline/src/integration/evidence-ledger.contract.ts`; in-memory and PostgreSQL suites. |
| HSD7-R-002 | Locally and CI verified: versioned immutable comparison evidence preserves required provenance and hashes without secrets. | `hotel_shoreline/src/unit/evidence-ledger/records.unit.test.ts` |
| HSD7-R-003 | Locally, CI, and externally verified: server-only repository configuration and sanitized history APIs expose typed failures only on the production merged-main revision. | `hotel_shoreline/src/unit/evidence-ledger/config.unit.test.ts`; `hotel_shoreline/src/unit/evidence-ledger/view.unit.test.ts`; `hotel_shoreline/e2e/evidence-ledger.spec.ts` |
| HSD7-R-004 | Locally and CI verified against PostgreSQL 17.8; the full repository contract executes through a dedicated role limited to `CONNECT`, schema `USAGE`, and table `SELECT`/`INSERT`, and the same authority is externally verified on Cloud SQL. | `hotel_shoreline/src/integration/evidence-ledger.postgres.integration.test.ts` |
| HSD7-O-001 | Externally verified: Cloud SQL shape, separate identities, pinned secrets, IAM boundary, least privilege, migration, persistence, cost, traffic rollback, and limitations are recorded. | `hotel_shoreline/CLOUD_SQL.md`; `hotel_shoreline/migrations/README.md` |
| HSD7-Q-001 | Locally, CI, and externally verified: PR #13 passed; merged-main revision `hotel-shoreline-hsd007-4f1a1d5` serves production and retrieved the pre-deployment comparison through Cloud SQL with no error-severity revision logs at closeout. | HSD-007 Completion Record. |
| HSD6-H-001 | Locally, CI, and externally verified: newest-first history and exact-record retrieval preserve current evidence across empty, missing, malformed, and unavailable states with typed 400/404/503 boundaries. | `hotel_shoreline/src/unit/evidence-ledger/view.unit.test.ts`; `hotel_shoreline/e2e/evidence-experience.spec.ts`; HSD-006 Completion Record |
| HSD6-U-001 | Locally, CI, and externally verified: arm lifecycle presentation distinguishes reached, failed, not-reached, partial/failed, completed, and pair-exclusion states without inventing events. | `hotel_shoreline/src/unit/evidence-experience.unit.test.ts`; `hotel_shoreline/e2e/evidence-experience.spec.ts`; HSD-006 Completion Record |
| HSD6-U-002 | Locally, CI, and externally verified: record-derived comparison inspection preserves source/provenance, matched conditions, evidence, measure definitions/denominators, first-loss, and claim limits. | `hotel_shoreline/src/unit/evidence-experience.unit.test.ts`; `hotel_shoreline/e2e/evidence-experience.spec.ts`; HSD-006 Completion Record |
| HSD6-U-003 | Locally, CI, and externally verified: one deterministic versioned JSON download is projected from immutable evidence and excludes secrets, server/database details, hidden prompts, exceptions, and real personal data. | `hotel_shoreline/src/unit/evidence-experience.unit.test.ts`; `hotel_shoreline/e2e/evidence-experience.spec.ts`; HSD-006 Completion Record |
| HSD6-U-004 | Locally, CI, and externally verified: desktop/390 px keyboard flow, focus, status announcements, overflow, failure states, disclosures, and console behavior remain usable. | `hotel_shoreline/e2e/evidence-experience.spec.ts`; HSD-006 Completion Record |
| HSD8-C-001 | Locally verified: the official deadline, Taskmaster/stack requirements, submission artifacts, judging weights, new-project disclosure, and freeze rules were re-read and dated on 2026-08-27. | `issues/HACKATHON-CONSTRAINTS.md`; HSD-008 Completion Record |
| HSD8-R-001 | Locally and CI verified: exact merged commit `b7fd869` installed and passed repository, audit, type, unit/integration/coverage/E2E, and build gates in a secret-free clean Git clone; PR #17 supplied PostgreSQL 17 proof. | HSD-008 clean-clone and CI records |
| HSD8-D-001, HSD8-D-002 | Locally and CI verified: the judge-facing root README and canonical submission package are machine-checked, reproducible, linked, disclosed, and consistent with the release. | `scripts/verify-release.mjs`; documentation review |
| HSD8-V-001 | Locally verified, owner recording pending: the 3:55 English-ready storyboard includes friction/value, an unedited live action, Cloud proof, architecture/evidence, safety limits, and no secret capture. | `docs/submission/README.md`; HSD-008 Completion Record |
| HSD8-Q-001 | Locally and CI verified: tracked release hygiene rejects credentials, local/generated state, duplicate artifacts, broken links, unsupported placeholders, and undisclosed licence/dependency risk. | `scripts/verify-release.mjs`; `pnpm check`; `pnpm audit:prod` |
| HSD8-O-001 | Externally verified: the public repository and merged-main Cloud Run revision pass URL, persisted-history/exact-export/400/404, desktop/390 px, configuration, traffic, and zero-error-log checks. | HSD-008 Completion Record |
| HSD8-F-001 | Implemented: final owner video/Devpost/tag/freeze actions are explicit and separate from completed repository work. | `docs/submission/README.md`; HSD-008 Completion Record |
